...
```{r}
plants = plants %>% pivot_longer(c(`height.0`, `height.7`, `height.14`, `height.21`), names_to = "height") %>%  separate(height, into = c("century", "Days"), sep = 7)  
...
```{r}
pokemon$rate = with(pokemon, reorder(type_1, -rate, mean))
