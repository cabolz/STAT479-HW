...
### Part A
```{r}
ggplot(ikea) + geom_point(aes(x = category, y = price)) + scale_y_log10() + geom_jitter(aes(x = category, y = price)) + theme(axis.text.x = element_text(angle = 60, hjust = 1)) + xlab("Furniture Category") + ylab("Log Price")
```
...
ggplot(ikea) + 
  ...
  xlab("Furniture Category") + 
  ylab("Log Price") + 
  geom_point(data = highlight_df, aes(x = category, y = price), color = 'red') + 
  geom_label_repel(data = highlight_df, aes(x = category, y = price), label = highlight_df$name, hjust =1, vjust = 0, col ='purple', size = 3, max.overlaps = 50)
```
